...
export function deleteTrainer(id) {
    ...
    return (dispatch) => {
        if (window.confirm(`Are You Sure Want To Delete Data?`)) {
            //   dispatch(getDeleteTrRequest());
            Delete(`${urls.trainers}/${id}`)
                .then(() => {
                    //   dispatch(getDeleteTrSuccess());
...
//single get 
export function getSingleTrRequest() {
    return {
        type:  actionType.GET_SINGLE_TRAINER_REQUEST,
    };
}

export function getSingleTrSuccess(data) {
    return {
        type: actionType.GET_SINGLE_TRAINER_SUCCESS,
        payload: data,
    };
}

export function getSingleTrError(error) {
    return {
        type: actionType.GET_SINGLE_TRAINER_ERROR,
        payload: error,
    };
}
...
export function getSingleTrainer(trainerId) {
    return (dispatch) => {
        dispatch(getSingleTrRequest());
        Get(`${urls.trainers}/${trainerId}`)
            .then((response) => {
                dispatch(getSingleTrSuccess(response.data));
            })
            .catch((error) => {
                dispatch(getSingleTrError(error.response));
            });
    };
}
...
export function editTarinerSuccess(data) {
    return {
...
export function editTrainer(editTrainerData, id) {
    return (dispatch) => {
        // Fetch the single branch data
        dispatch(getSingleTrainer(id));

        dispatch(editTrainerRequest());
        Put(`${urls.trainers}/${id}`, editTrainerData)
            .then((response) => {
                dispatch(editTarinerSuccess(response.data));
